...
cleanschedule <- function(whichsched){
  ...
  # Loading files
  sched.meta <- read.xlsx(paste("ET_data/6.USA_FTAs_indiv_spreadsheet/", 
                                whichsched, sep=""), 
                          sheet = 1, colNames=TRUE, rowNames=TRUE) # read sheet 1: metadata (updated 3/12/24 ET)
  sched.full <- read.xlsx(paste("ET_data/6.USA_FTAs_indiv_spreadsheet/", 
                                whichsched, sep=""), 
                          sheet = 2) # read sheet 2: the actual product-level tariff schedule
  sched.cat <- read.xlsx(paste("ET_data/6.USA_FTAs_indiv_spreadsheet/", 
                               whichsched, sep=""), 
