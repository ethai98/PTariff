...
cleanschedule_NAFTA <- function(whichsched){
  # whichsched <- "543_USA_JOR_gpt_handcoded.xlsx"
  require(openxlsx)
  require(tidyverse)

  
  # Loading files
  sched.meta <- read.xlsx(paste("ET_data/6.NAFTA_indiv_spreadsheet/", 
                                whichsched, sep=""), 
                          sheet = 1, colNames=TRUE, rowNames=TRUE) # read sheet 1: metadata (updated 3/12/24 ET)
  sched.full <- read.xlsx(paste("ET_data/6.NAFTA_indiv_spreadsheet/", 
                                whichsched, sep=""), 
                          sheet = 2) # read sheet 2: the actual product-level tariff schedule
  sched.cat <- read.xlsx(paste("ET_data/6.NAFTA_indiv_spreadsheet/", 
                               whichsched, sep=""), 
                         sheet = 3, colNames=TRUE, rowNames=FALSE) # read sheet 3: categories (updated 3/12/24 ET)
  
  # separate_rows() to expand the dataset for each category a product code has
  ...
  hs.cor <- read.xlsx("EVL_data/hs_codes/hs_correspondence.xlsx")
  ...
  cat.formerge <- data.frame(category = sched.cat$cat_code,
                             sched.cat[,2:8]) # adjust here if has more sched.cat column to add
  
  # merging details about what a category means to the schedule
  sched <- merge(sched, cat.formerge, by = "category", all.x=TRUE)
  
  # 9/12/24 summarise back to product-code level
  
  sched <- sched %>%
    group_by(code) %>%
    # fill in 0 for years_pause
    mutate(years_pause = ifelse(is.na(years_pause), 0, years_pause)) %>%
    summarise(category = paste(unique(category), collapse = ", "),
              baserate = max(baserate),
              details = max(details),
              reduced = max(reduced),
              removed = max(removed),
              immediate = max(immediate),
              years_delay = mean(years_delay, na.rm=T),
              years_delay = ifelse(is.nan(years_delay), NA, years_delay),
              equal_steps = max(equal_steps, na.rm=T),
              equal_steps = ifelse(is.infinite(equal_steps), NA, equal_steps),
              backloaded = max(backloaded, na.rm=T),
              backloaded = ifelse(is.infinite(backloaded), NA, backloaded),
              years_pause = mean(years_pause, na.rm=T),
              years_pause = ifelse(is.nan(years_pause), NA, years_pause)) %>%
      # take out years_pause where there were no backloaded coded (when it's NA or != 1)
    mutate(years_pause = ifelse(backloaded != 1, NA, years_pause))
  ...
  colnames(sched) <- c(colnames(sched)[1:14],  # maybe need to change this if I'm including backloaded and years_delay
  ...
  for(v in c("reduced", "removed", "immediate", "years_delay", "equal_steps", "backloaded", "years_pause", # added backloaded and years_pause
