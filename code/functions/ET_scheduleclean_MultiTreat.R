...
cleanschedule_MultiTreat <- function(whichsched){
  ...
  # Loading files
  sched.meta <- read.xlsx(paste("ET_data/6.MultiTreat_indiv_spreadsheet/", 
                                whichsched, sep=""), 
                          sheet = 1, colNames=TRUE, rowNames=TRUE) # read sheet 1: metadata (updated 3/12/24 ET)
  sched.full <- read.xlsx(paste("ET_data/6.MultiTreat_indiv_spreadsheet/", 
                                whichsched, sep=""), 
                          sheet = 2) # read sheet 2: the actual product-level tariff schedule
  sched.cat <- read.xlsx(paste("ET_data/6.MultiTreat_indiv_spreadsheet/", 
                               whichsched, sep=""), 
  ...
  sched.full <- sched.full %>%
    separate_rows(category, baserate, sep = ", ") %>%
    mutate(baserate = as.numeric(baserate))
  ...
  sched <- sched %>%
    ...
    summarise(category = paste(unique(category), collapse = ", "),
              baserate = mean(baserate, na.rm=T),
              details = max(details),
              reduced = max(reduced),
              removed = max(removed),
  ...
  colnames(sched) <- c(colnames(sched)[1:15],  # maybe need to change this if I'm including backloaded and years_delay, add the number of rows
