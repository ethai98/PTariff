...
input_folder_path <- here("3.gpt_processed_schedule/EVL_remaining")  # Replace 'your_input_folder_name' with the name of your input folder

# Define the path to the output folder
output_folder_path <- here("4.metadata_processed_schedule/EVL_remaining")
...
convert_csv_to_excel <- function(file_path) {
  ...
  categories_headers <- c("cat_code", "reduced", "removed", "immediate", "years_delay", "equal_steps", "backloaded", "years_pause", "cdf", "path", "note")
...
fta_list <- read_csv("ET_file/ALL_FTA_list.csv") %>%
...
update_categories_with_unique_codes <- function(file_path) {
  ...
  addFilter(wb, sheet = 3, cols = 1:11, rows = 1)
  addFilter(wb, sheet = 2, cols = 1:5, rows = 1)
